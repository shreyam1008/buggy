...
// ============================================================================
// PHOTO UPLOAD EDITOR COMPONENT
// ============================================================================

function PhotoUploadEditor({ onPhotoReady, initialPhoto = null }: { onPhotoReady: (compressed: any) => void; initialPhoto?: string | null }) {
  const [mode, setMode] = useState<'select' | 'webcam' | 'edit'>('select');
  const [capturedImage, setCapturedImage] = useState<string | null>(initialPhoto);
...
function AddSingleDevotee({ onNavigate }) {
  ...
  const handlePhotoReady = (compressed: any) => {
    setFormData({ ...formData, photo: new File([compressed.blob], 'photo.jpg', { type: 'image/jpeg' }) });
          ...
          <div className="space-y-4">
            ...
            {photoPreview ? (
              <div className="space-y-4">
                ...
                <Button type="button" variant="secondary" onClick={() => { setPhotoPreview(null); setPhotoSize(0); setFormData({ ...formData, photo: null }); }}>
                  Change Photo
                </Button>
              </div>
            ) : (
